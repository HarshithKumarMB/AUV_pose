...
def two_components():
  log_amplitude = torch.log(torch.tensor([4.0, 0.5], dtype=torch.float64))
  log_lengthscale = torch.log(
    torch.tensor([[6.0, 9.0], [0.8, 1.1]], dtype=torch.float64)
  )
  return log_amplitude, log_lengthscale


def test_a_stacked_kernel_is_the_sum_of_its_components():
  a = torch.randn(7, 2, dtype=torch.float64) * 5
  b = torch.randn(5, 2, dtype=torch.float64) * 5
  log_amplitude, log_lengthscale = two_components()

  expected = matern52(a, b, log_amplitude[0], log_lengthscale[0]) + matern52(
    a, b, log_amplitude[1], log_lengthscale[1]
  )
  torch.testing.assert_close(
    matern52(a, b, log_amplitude, log_lengthscale), expected
  )


def test_a_stacked_gradient_is_the_sum_of_its_components():
  a = torch.randn(7, 2, dtype=torch.float64) * 5
  b = torch.randn(5, 2, dtype=torch.float64) * 5
  log_amplitude, log_lengthscale = two_components()

  expected = matern52_gradient(
    a, b, log_amplitude[0], log_lengthscale[0]
  ) + matern52_gradient(a, b, log_amplitude[1], log_lengthscale[1])
  torch.testing.assert_close(
    matern52_gradient(a, b, log_amplitude, log_lengthscale), expected
  )


def test_a_stacked_kernel_differentiates_through_both_components():
  a = torch.randn(6, 2, dtype=torch.float64) * 3
  log_amplitude, log_lengthscale = two_components()
  log_amplitude.requires_grad_(True)
  log_lengthscale.requires_grad_(True)

  matern52(a, a, log_amplitude, log_lengthscale).sum().backward()
  assert log_amplitude.grad is not None and log_lengthscale.grad is not None
  assert torch.all(log_amplitude.grad != 0)
  assert torch.all(log_lengthscale.grad != 0)
