...
def test_the_map_does_not_depend_on_where_the_survey_sits(synthetic):
  """Standardising is internal: shifting the survey shifts the map, nothing more.

  Survey coordinates can be kilometres from the origin; in float32 without
  centring, that would cost the fit its resolution.
  """
  X, y = synthetic
...
def test_the_gradient_is_in_metres_per_metre_not_standardised_units():
  ...
  This is what catches a missing ``x_scale``. Omitting it leaves a
