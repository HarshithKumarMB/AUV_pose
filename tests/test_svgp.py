...
def test_fit_is_reproducible_given_a_seed(synthetic):
    a = fit_svgp(synthetic["train_x"], synthetic["train_y"], n_inducing=10, epochs=1, seed=7)[2]
    b = fit_svgp(synthetic["train_x"], synthetic["train_y"], n_inducing=10, epochs=1, seed=7)[2]
    torch.testing.assert_close(a, b)


def test_fit_rejects_more_inducing_points_than_data(synthetic):
    with pytest.raises(ValueError, match="exceeds"):
        fit_svgp(synthetic["train_x"], synthetic["train_y"], n_inducing=10_000, epochs=1)


def _fitted_map(synthetic, epochs=15):
    model, likelihood, _ = fit_svgp(
        synthetic["train_x"], synthetic["train_y"], n_inducing=32, epochs=epochs, seed=0
    )
    return BathymetryMap(
        model, likelihood, synthetic["x_scaler"], synthetic["y_mean"], synthetic["y_std"]
    )
...
def test_checkpoint_roundtrip(tmp_path, synthetic):
    ...
    save_map(
        path, model, likelihood, inducing,
        synthetic["x_scaler"], synthetic["y_mean"], synthetic["y_std"],
    )
