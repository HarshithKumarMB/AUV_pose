...
def test_missing_column_is_rejected(tmp_path):
  with (
    CsvWriter(tmp_path / "log.csv", COLUMNS) as log,
    pytest.raises(ValueError, match=r"lacks \['y', 'z'\]"),
  ):
...
def test_finished_rows_are_on_disk_before_exit(tmp_path):
  """A simulator run that is killed never reaches __exit__."""
  path = tmp_path / "log.csv"
  with CsvWriter(path, COLUMNS) as log:
    log.write(step=0, x=1.0, y=2.0, z=3.0)
    assert read(path) == [list(COLUMNS), ["0", "1.0", "2.0", "3.0"]]


def test_a_refused_row_writes_nothing(tmp_path):
  path = tmp_path / "log.csv"
  with CsvWriter(path, COLUMNS) as log:
