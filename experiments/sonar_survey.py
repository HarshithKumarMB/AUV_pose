...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sonar", choices=["imaging", "sidescan"], default="sidescan")
    parser.add_argument("--world", default="Dam")
    parser.add_argument("--start", type=float, nargs=3, default=[1.0, 2.0, -6.0])
    parser.add_argument("--steps", type=int, default=2000)
    parser.add_argument("--command", type=float, nargs=8,
...
def main() -> None:
    ...
    env = holoocean.make(scenario_cfg=build_scenario(args.sonar, args.world, args.start))
    state = env.tick()
