...
def add_projected_position(matched: pd.DataFrame) -> pd.DataFrame:
    ...
    matched["calc_y"] = matched["nearest_y"] - matched["r"] * np.sin(matched["theta"])
    matched["calc_z"] = matched["nearest_z"] + matched["r"] * np.cos(matched["theta"])
    return matched
...
def main() -> None:
    ...
    if args.plot and all_points:
        ...
        axes.scatter(points["calc_x"], points["calc_y"], c="green", s=10,
