...
SIDESCAN = dict(range_min=0.5, range_max=70.0, range_bins=256, azimuth_bins=256)
...
def project_returns(
    profile: np.ndarray, position: np.ndarray, ranges: np.ndarray, angles: np.ndarray
) -> np.ndarray:
    ...
    local = np.column_stack([np.zeros_like(r), r * np.sin(theta), -r * np.cos(theta)])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--classifier", type=Path, default=Path("obstacle_classifier.pkl"))
