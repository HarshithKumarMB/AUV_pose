...
def plot_plane(frame: pd.DataFrame, plane: str, path: Path) -> None:
    ...
    for prefix, label, colour, style, width in TRACKS:
        ...
        if not set(columns) <= set(frame.columns):
            continue
        axes.plot(
            frame[columns[0]], frame[columns[1]],
            label=label, color=colour, linestyle=style, linewidth=width,
        )

    axes.scatter(frame[horizontal].iloc[0], frame[vertical].iloc[0],
                 color="black", s=100, marker="o", label="Start", zorder=5)
    axes.scatter(frame[horizontal].iloc[-1], frame[vertical].iloc[-1],
                 color="purple", s=100, marker="x", label="End", zorder=5)
...
def plot_error(frame: pd.DataFrame, path: Path) -> None:
    ...
    for prefix, label, colour in (("ekf_", "EKF", "green"), ("dr_", "Dead reckoning", "red")):
        if not {prefix + a for a in "xyz"} <= set(frame.columns):
            continue
        error = error_norm(frame, prefix)
        axes.plot(steps, error, label=f"{label} (RMS {np.sqrt((error**2).mean()):.2f} m)",
...
def main() -> None:
    ...
    for prefix, label in (("ekf_", "EKF"), ("dr_", "Dead reckoning")):
        if {prefix + a for a in "xyz"} <= set(frame.columns):
            error = error_norm(frame, prefix)
            print(f"{label:16s} RMS {error.pow(2).mean() ** 0.5:7.3f} m   "
